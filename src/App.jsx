...
function SelectField({ value, onChange, options, placeholder }) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 outline-none focus:border-slate-400">
...
function OrderForm({ onSave, initialValues, onCancel, clients, saving }) {
  ...
  const handleClientSelect = (name) => {
    ...
    setForm((current) => ({ ...current, cliente: name, cidade: client?.cidade || current.cidade, estado: client?.estado || current.estado }));
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    onSave({ ...form, pedido: Number(form.pedido), qtd: Number(form.qtd || 0), prazoEntrega: Number(form.prazoEntrega || 0) });
  };

  return (
    <form onSubmit={handleSubmit} className="grid gap-4">
      <div className="grid gap-4 md:grid-cols-2">
        <div className="grid gap-2"><Label>Número do pedido</Label><Input value={form.pedido} onChange={(e) => updateField("pedido", e.target.value)} required /></div>
        <div className="grid gap-2">
          <Label>Cliente</Label>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="grid gap-2"><Label>Item</Label><Input value={form.item} onChange={(e) => updateField("item", e.target.value)} required /></div>
        <div className="grid gap-2"><Label>Quantidade</Label><Input type="number" value={form.qtd} onChange={(e) => updateField("qtd", e.target.value)} required /></div>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        <div className="grid gap-2"><Label>Situação</Label><SelectField value={form.situacao} onChange={(value) => updateField("situacao", value)} options={["Em Aberto", "Em Andamento", "Arte enviada", "Finalizado"]} /></div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="grid gap-2"><Label>Data do pedido</Label><Input type="date" value={form.dataPedido} onChange={(e) => updateField("dataPedido", e.target.value)} /></div>
        <div className="grid gap-2"><Label>Referência</Label><Input type="date" value={form.referencia} onChange={(e) => updateField("referencia", e.target.value)} /></div>
        <div className="grid gap-2"><Label>Data da festa</Label><Input type="date" value={form.dataFesta} onChange={(e) => updateField("dataFesta", e.target.value)} /></div>
        <div className="grid gap-2"><Label>Prazo de entrega</Label><Input type="number" value={form.prazoEntrega} onChange={(e) => updateField("prazoEntrega", e.target.value)} /></div>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        <div className="grid gap-2"><Label>Observações do pedido</Label><Textarea rows={4} value={form.observacoesPedido} onChange={(e) => updateField("observacoesPedido", e.target.value)} /></div>
        <div className="grid gap-2"><Label>Observações internas</Label><Textarea rows={4} value={form.observacoesInternas} onChange={(e) => updateField("observacoesInternas", e.target.value)} /></div>
      </div>
      <div className="flex gap-2 pt-2">
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid w-full grid-cols-3 rounded-2xl bg-white p-1 shadow-sm md:w-[420px]">
          {[
            ...
            <button key={key} onClick={() => setTab(key)} className={cls("rounded-xl px-3 py-2 text-sm font-medium", tab === key ? "bg-slate-900 text-white" : "text-slate-700")}>{label}</button>
          ))}
        </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.5fr_0.9fr]">
            <Card>
              <div className="p-6">
                ...
                <div className="mt-4">
                  {loading ? <div className="flex items-center gap-2 text-slate-500"><Loader2 className="h-4 w-4 animate-spin" /> Carregando pedidos...</div> : (
                            ...
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                              ...
                              <div className="grid min-w-[300px] gap-2 text-sm text-slate-600">
                                ...
                                <div className="flex flex-wrap gap-2 pt-2">
                                  {whatsappLink && <Button variant="outline" className="rounded-xl" as="a" onClick={() => window.open(whatsappLink, "_blank", "noopener,noreferrer")}><MessageCircle className="mr-2 h-4 w-4" /> WhatsApp</Button>}
          ...
          <div className="grid gap-6 xl:grid-cols-[1.25fr_0.85fr]">
            <Card>
              <div className="p-6">
                ...
                <div className="mt-3"><Input placeholder="Buscar por nome, cidade, telefone ou e-mail" value={clientSearch} onChange={(e) => setClientSearch(e.target.value)} /></div>
                        ...
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          ...
                          <div className="min-w-[220px] space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              <Button variant="outline" className="rounded-xl" onClick={() => { setEditingClient(client); setClientOpen(true); }}><PencilLine className="mr-2 h-4 w-4" /> Editar</Button>
                              <Button variant="outline" className="rounded-xl" onClick={() => deleteClient(client.id)}><Trash2 className="mr-2 h-4 w-4" /> Excluir</Button>
                            </div>
                          </div>
                        </div>
                      </div>
