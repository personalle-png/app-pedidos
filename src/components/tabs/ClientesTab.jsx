...
export default function ClientesTab({ clientSearch, setClientSearch, filteredClients, orders, setEditingClient, setClientOpen, deleteClient, clients }) {
  ...
  <Button onClick={() => setClientOpen(true)}>
    + Novo cliente
          ...
          <div className="mt-3">
            <Input placeholder="Buscar por nome, CPF, CEP, endereço, cidade, telefone ou e-mail" value={clientSearch} onChange={(e) => setClientSearch(e.target.value)} />
          </div>
                ...
                <div key={client.id} className="rounded-3xl border border-slate-200 bg-white p-4">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="space-y-2">
                      ...
                      {client.endereco && <p className="text-sm text-slate-600">{client.endereco}{client.numero ? `, ${client.numero}` : ''}</p>}
                      ...
                      {client.complemento && <p className="text-sm text-slate-600">Complemento: {client.complemento}</p>}
