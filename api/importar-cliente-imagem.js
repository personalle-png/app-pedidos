...
export default function ImportarClienteImagem({ onConfirmImport }) {
  ...
  const imageName1 = useMemo(
    () => imageFile1?.name || "Nenhuma imagem selecionada",
    [imageFile1]
  );

  const imageName2 = useMemo(
    () => imageFile2?.name || "Nenhuma imagem selecionada",
    [imageFile2]
  );
  ...
  const handleReadImage = async () => {
    ...
    try {
      ...
      if (imageFile1) {
        images.push(await fileToBase64(imageFile1));
      }

      if (tipoImportacao === "elo7" && imageFile2) {
        images.push(await fileToBase64(imageFile2));
      }
      ...
      const mergedData =
        tipoImportacao === "elo7" && Array.isArray(data?.items)
          ? mergePreferFirst(data.items[0], data.items[1])
          : data;
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setImportError(err instanceof Error ? err.message : "Erro ao importar cliente.");
    }
  };

  const UploadBox = ({ title, fileName, previewUrl, onChange }) => (
...
"""^
# trim accidental tail? not possible here
print("placeholder")
# rewrite without tail
frontend = frontend.split('"""^')[0]
backend = """import OpenAI from \"openai\";
...
"""
p1 = "/mnt/data/importar_cliente_imagem_multi_origem.jsx"
p2 = "/mnt/data/api_importar_cliente_imagem_multi_origem.js"
